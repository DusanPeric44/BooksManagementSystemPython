...
def get_books():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, title, author_id, books_link, genres, average_rating, published_year FROM books")
...
def create_book(book: BookCreate, _: str = Depends(get_api_key)):
    ...
    try:
        ...
        cursor.execute("INSERT INTO books (title, author_id, books_link, genres, average_rating, published_year) "
                       "VALUES (?, ?, ?, ?, ?, ?)",
...
def update_book(book_id: int, book: BookCreate, _: str = Depends(get_api_key)):
    ...
    genres = ",".join(book.genres)  # Convert list of genre names to a comma-separated string
    cursor.execute(
        "UPDATE books SET title = ?, author_id = ?, books_link = ?, genres = ?, average_rating = ?, published_year = ? "
        "WHERE id = ?",
